...
def create_song(client: TestClient) -> dict[str, Any]:
    response = client.post(f"/api/organizations/{client.organization_id}/admin/songs", json={"title": "Manifest Song", "slug": "manifest-song"})
...
def mark_stem_ready(client: TestClient, song_id: int, stem_id: int, *, write_file: bool = True) -> Path:
    ...
    converted_path = storage.converted_path(client.organization_id, song_id, stem_id)  # type: ignore[attr-defined]
    if write_file:
...
def test_manifest_includes_ready_playback_stems(client: TestClient) -> None:
    ...
    response = client.get(f"/api/organizations/{client.organization_id}/admin/songs/{song['id']}/manifest")
    ...
    assert media_url.startswith(
        f"/media/organizations/{client.organization_id}/songs/{song['id']}/keys/{manifest['selectedKeyId']}/stems/{ready_stem['id']}.m4a?v="
    )
...
def test_partial_song_is_publicly_playable_with_ready_stem(client: TestClient) -> None:
    ...
    list_response = client.get(f"/api/organizations/{client.organization_id}/songs")
    ...
    response = client.get(f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest")
    assert response.status_code == 200
    manifest = response.json()
    assert manifest["playable"] is True
...
def test_partial_key_variant_is_playable_and_empty_variant_is_not(client: TestClient) -> None:
    ...
    response = client.get(f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest", params={"key": 2})
    assert response.status_code == 200
    manifest = response.json()
    ...
    compatibility_response = client.get(
        f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest",
        params={"keyId": partial_key_id},
    )
    assert compatibility_response.status_code == 200
    assert compatibility_response.json()["selectedKeyId"] == partial_key_id

    unavailable_response = client.get(f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest", params={"key": 4})
    assert unavailable_response.status_code == 404
    assert unavailable_response.json()["detail"] == "Tonart nicht verfügbar"

    missing_response = client.get(f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest", params={"key": 6})
...
def test_public_manifest_rejects_key_and_key_id_together(client: TestClient) -> None:
    ...
    manifest = client.get(f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest").json()
...
def test_public_manifest_validates_musical_key_range(client: TestClient) -> None:
    ...
    assert client.get(f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest", params={"key": "x"}).status_code == 422
    assert client.get(f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest", params={"key": 12}).status_code == 422
...
def test_keyless_stem_uses_original_asset_for_every_existing_key(client: TestClient) -> None:
    ...
    response = client.get(f"/api/organizations/{client.organization_id}/songs/{song['id']}/manifest", params={"keyId": target_key_id})
    assert response.status_code == 200
    manifest = response.json()
    assert manifest["playable"] is True
    ...
    assert manifest["stems"][0]["url"].startswith(
        f"/media/organizations/{client.organization_id}/songs/{song['id']}/keys/{original_key_id}/stems/{drums['id']}.m4a?v="
    )
    assert client.get(manifest["stems"][0]["url"]).status_code == 200

    inventory = client.get(f"/api/organizations/{client.organization_id}/admin/songs/{song['id']}/key-assets").json()
...
def test_media_serves_ready_m4a_with_headers(client: TestClient) -> None:
    ...
    assert response.headers["cache-control"] == "private, no-cache, max-age=315360000"
...
def test_media_cache_max_age_can_be_configured(client: TestClient) -> None:
    ...
    assert response.headers["cache-control"] == "private, no-cache, max-age=60"


def test_media_requires_reauthorization_after_leaving_organization(client: TestClient) -> None:
    song = create_song(client)
    stem = upload_stem(client, song["id"])
    mark_stem_ready(client, song["id"], stem["id"])
    manifest = client.get(
        f"/api/organizations/{client.organization_id}/admin/songs/{song['id']}/manifest"
    ).json()
    media_url = manifest["stems"][0]["url"]

    assert client.get(media_url).status_code == 200
    assert client.delete(f"/api/organizations/{client.organization_id}/session").status_code == 204
...
def test_manifest_media_url_changes_when_asset_is_updated(client: TestClient) -> None:
    ...
    manifest_url = f"/api/organizations/{client.organization_id}/admin/songs/{song['id']}/manifest"
...
def test_media_rejects_missing_and_not_ready_files(client: TestClient) -> None:
    ...
    not_ready_response = client.get(f"/media/organizations/{client.organization_id}/songs/{song['id']}/keys/999/stems/{stem['id']}.m4a")
    assert not_ready_response.status_code == 404

    mark_stem_ready(client, song["id"], stem["id"], write_file=False)
    manifest = client.get(f"/api/organizations/{client.organization_id}/admin/songs/{song['id']}/manifest").json()
    missing_file_response = client.get(f"/media/organizations/{client.organization_id}/songs/{song['id']}/keys/{manifest['selectedKeyId']}/stems/{stem['id']}.m4a")
    assert missing_file_response.status_code == 404

    wrong_song_response = client.get(f"/media/organizations/{client.organization_id}/songs/{song['id'] + 1}/keys/{manifest['selectedKeyId']}/stems/{stem['id']}.m4a")
